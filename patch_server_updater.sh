...
# INIT COMMAND HANDLER
  if [ "$1" == "init" ]; then
    OPTIND=2
    while getopts "j:u:o:t:h" param; do
      case "${param}" in
        j)
          echo "JSON : $OPTARG"
          JSON_INPUT="$OPTARG"
...
# ADD COMMAND HANDLER
    ...
    OPTIND=2
    while getopts "d:v:o:c:t:h" param; do
      case "${param}" in
        d)
          echo "DATE : $OPTARG"
          DATE_INPUT="$OPTARG"
...
usage_init() {
echo -n "
$SCRIPT_NAME init -j [TEMPLATE_FILE] -u [PATCH_SERVER_URL] -o [OUTPUT_FILE_CONFIG] -t [API_TOKEN]
...
usage_add () {
echo -n "
$SCRIPT_NAME add -d [DATE] -v [VERSION] -o [OS_MIN] -c [CONFIG_FILE] -t [API_TOKEN]
...
check_newversion () {
		...
		curl -H "Authorization: Bearer $CONFIG_TOKEN" --silent -m 60 --output /dev/null -X POST $PATCH_SERVER_API_URL_POST_VERSION -d "$data_to_push" -H 'Content-Type: application/json'
		echo "Done"
		#slack_alert
	else
